...
public class UserSolution
{
    public string Execute()
    {
        ...
        {
            string tempFile = Path.GetTempFileName() + ".php";
            try
            {
                File.WriteAllText(tempFile, code);
                string phpExe = "php.exe";
                ...
                {
                    string output = await Task.Run(() => p.StandardOutput.ReadToEnd());
                    string error = await Task.Run(() => p.StandardError.ReadToEnd());
                    p.WaitForExit(5000);
                    if (p.ExitCode != 0)
                        return $"Ошибка PHP: {error}";
                    return output;
                }
            }
            finally
